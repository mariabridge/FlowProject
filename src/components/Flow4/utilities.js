...
const graphUtils = {
	...
	buildFlowGraph11:buildFlowGraph11,
 ...
 */
function StoreFlowGraph(project_name, project_owner, project_id) {
	...
	forEach(flows, function(value, key) {
		...
		flowObj[k]['title'] = value.title;
		flowObj[k]['desc'] = value.description;
		flowObj[k]['childNodes'] = [];
		flowObj[k]['connectors'] = [];
		let connectorIds = [];


		if (value.nodeIDs.length) {// NodeIds inserted in localstorage flows
			// System, Target and Source & Microflows
			for(var n=0; n<value.nodeIDs.length; n++){

				var nodeID = value.nodeIDs[n];
				...
				for(let i = 0; i < Object.keys(allCells).length; i++) {
					...
					if(value.nodeIDs.includes(node.id))
					{
						if( node.id == nodeID ){
						// Insert Flows or Integrations
					...
					//	console.log(node);
							...
							if(node.value.nodeName == 'Flow')
							{			
								flowObj[k]['id'] = node.getAttribute("elementId");	
								flowObj[k]['type'] = node.value.nodeName;
								flowObj[k]['x'] = node.geometry.x;
								flowObj[k]['y'] = node.geometry.y;
								flowObj[k]['width'] = node.geometry.width;
								flowObj[k]['height'] = node.geometry.height;
								flowObj[k]['cell_id'] = parseInt(node.id);
								flowObj[k]['style'] = node.style;
								flowObj[k]['endPoints'] = endPoints;
								flowObj[k]['integration_id'] = node.getAttribute("flowId");;
							...
							{
								let childnode = {};
								childnode['id'] = node.getAttribute("elementId");
								childnode['title'] = node.getAttribute("title");
								childnode['type'] = node.value.nodeName;
								childnode['x'] = node.geometry.x;
								childnode['y'] = node.geometry.y;
								childnode['width'] = node.geometry.width;
								childnode['height'] = node.geometry.height;
								childnode['cell_id'] = parseInt(node.id);
								childnode['style'] = node.style;
								childnode['endPoints'] = endPoints;
								totalNodes++;
								flowObj[k]['childNodes'].push(childnode);
							}					
						}
					}
					else
					{
						...
						if(!connectorIds.includes(node.id))
						{
							let connector = {};

							if(node.source)
							{
								connector['id'] = node.value;
								connector["title"] = 'connector';
								connector['type'] = 'connector';
								connector['source'] = parseInt(node.source.id);
								connector['target'] = parseInt(node.target.id);
								connector['cell_id'] = parseInt(node.id);
								connector['style'] = node.style;

								if(value.nodeIDs.includes(node.source.id) && value.nodeIDs.includes(node.target.id))
								{
									flowObj[k]['connectors'].push(connector);

									connectorIds.push(node.id);
								}
		...
		//flowObj.push(flowObj['key']);
		k++;
	});
	
	axios.post( '/api/post/project', {project_name :project_name, user_id: project_owner ,flowObj : flowObj, totalNodes : totalNodes, project_id : project_id } ).then(function(response){
 ...
 */
function buildFlowGraph(graph, nodes, flowIDs, allNodes) {
	...
	for(let f=0; f<flowIDs.length; f++)
	{
		integrationId = flowIDs[f];
		...
		{
			node = nodes[i];
			flowId = node.getAttribute("flowId");
//console.log("flowId---"+flowId+'---Integrationid---'+integrationId);

			if(flowId == integrationId)
			{ // If the node belongs to the flow
				...
				if (node.nodeName == "Flow") {
					...
					newFlow.title = flowTitle = nodeTitle;
					newFlow.description = "Default description";

					newFlow.nodeIDs.push(nodeId);
					
				}
				else if ( node.nodeName === "Experience" || node.nodeName === "Process" || node.nodeName === "Information") {

					parent = microLayer;
					newFlow.microflows.push(nodeId);
					newFlow.nodeIDs.push(nodeId);
				}
				else
				{
				...
				try {
					...
					v1.setId(nodeId);
					//graph.model.getCell(nodeId).refresh();
			
					// Adds a child vertex to v1 which is the name of the node
					const label = graph.insertVertex(v1, null, nodeTitle, 0.5, 0.85, 0, 0, "title2", true );
		...
		newFlow.nodeIDs = uniq(newFlow.nodeIDs);
		
		flows[flowTitle] = newFlow;
		...
		try {
			...
			if(endPoints)
			{
				...
				for( let k=0; k<endPointArray.length;k++)
				{
					...
					{
						if( ((connectors[n].posX == nodeId) && (connectors[n].posY == parseInt(endPointArray[k]))) || ((connectors[n].posY == nodeId) && (connectors[n].posX == parseInt(endPointArray[k]))))
						{
							newNodeId = connectors[n].cell_id;
						}
					}
	...
	console.log(window.localStorage.flows);
	
}



function buildFlowGraph11(graph, nodes, flowIDs) {
	...
	for(let f=0; f<flowIDs.length; f++)
	{
		...
		for( let i=0; i<nodes.length; i++)
		{
			...
			if(!plottedNodes.includes(nodeId))
			{
				plottedNodes.push(nodeId);

				if (node.nodeName == "Connector") {
					edgeSource = parseFloat(node.getAttribute("source"));
					edgeTarget = parseFloat(node.getAttribute("target"));
				}
				else {
					nodeX = parseFloat(node.getAttribute("x"));
					nodeY = parseFloat(node.getAttribute("y"));
				...
				try {
					...
					{
						e1 = graph.insertEdge(parent, null, elementId, connections[edgeSource], connections[edgeTarget]);

						//console.log(connections[edgeSource]);
						//console.log(connections[edgeTarget]);
						e1.setId(nodeId);
					}
			...
			{ // If the node belongs to the flow
				...
				else if ( node.nodeName === "Source" || node.nodeName === "Target" )
				{
					newFlow.nodeIDs.push(nodeId);
				}
			}
