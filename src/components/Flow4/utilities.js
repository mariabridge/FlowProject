...
function StoreFlowGraph(project_name) {
	...
	for(let i = 0; i < Object.keys(allCells).length; i++) {
		...
		if( node.style != "title2" && node.value ){
			...
			if(node.parent.edges){
				for(let j =0; j< node.parent.edges.length;j++){
					endPoints.push( node.parent.edges[j].id);
				}
			}
	...
	// POST project data in to database
...
function CreateFlowGraph(graph, sidebar, node) {
	let image = null;
	let style = null;
	let title1 = null;
	let title2 = null;
	...
	try {
		...
		v1 = graph.insertVertex(parent, null, newNode, 50, 20, 100, 100, style);
	...
	for( let i=0; i<nodes.length; i++)
	{
		...
		try {
			...
			if(endPoints)
			{
				...
				{
					var e1 = graph.insertEdge(parent, null, '', connections[nodeId], connections[endPoints[j]]);
				}
			}
...
function addSidebarIcon(graph, sidebar, node) {
	...
	const funct = function(graph, evt, cell, x, y) {
		...
		try {
			...
			console.log("width---"+styleConstants.cell.width+"-------"+ styleConstants.cell.height)
			// Insert a vertex at the dropped position
