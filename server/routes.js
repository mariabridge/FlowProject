...
module.exports = function(app, passport) {
	...
	app.post('/api/post/registration', function (req, res) {
		...
		var sql = "INSERT INTO users (firstname, surname, email, business, roll, pass) VALUES ('"+req.body.firstname+"', '"+req.body.surname+"', '"+req.body.mail+"', '"+req.body.business+"', '"+req.body.roll+"', '"+password+"')";
	...
	app.post('/api/post/login', function (req, res) {
		...
		var sql = "SELECT * FROM users WHERE email = '"+req.body.email+"' AND pass = '"+password+"' ";
		con.query(sql, function (err, result) {
			if (err) {
				//throw err;
				res.send(err)
	...
	function getChild(flowObj, inte_id, res){
		for( var j=0; j<flowObj.length; j++ ){
			if( flowObj[j].style == "source" || flowObj[j].style == "target" ){
				var queryStr = "INSERT INTO FL_system (sys_cell_id, integration_id, sys_name, sys_posX, sys_posY, sys_width, sys_height, sys_style, sys_endpoints, sys_type)";
			}else {
				var queryStr = "INSERT INTO FL_microflow (mf_cell_id, integration_id, mf_name, mf_posX, mf_posY, mf_width, mf_height, mf_style, mf_endpoints, mf_type)";
			}
				
			var sql = queryStr+" VALUES ('"+
			flowObj[j].cell_id+"', '"+inte_id+"', '"+flowObj[j].title+"', '"+flowObj[j].x+"', '"+flowObj[j].y+"', '"+
			flowObj[j].width+"', '"+flowObj[j].height+"', '"+flowObj[j].style+"', '"+flowObj[j].endPoints+"', '"+flowObj[j].type+"')";
			con.query(sql, function (err, result) {
				if (err) {
					res.send(err)
				}else{
