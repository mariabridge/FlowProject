...
module.exports = function(app, passport) {
	...
	app.post('/api/post/project', function (req, res) {
		...
		var flowObj = req.body.flowObj;console.log(flowObj);

		sequelize.sync().then(function() {
			return Projects.create(req.body);
		}).then(function(jane) {
			var proj_id = jane.get({ plain: true }).id;
			...
			for( var i=0; i<flowObj.length; i++ ){
				if(flowObj[i].style == "flow"){
					...
					fillIntegration(flowObj[i], proj_id, flowObj);
				}
			}
		});
	...
	function fillIntegration(flowObj, proj_id, flowObjs){
		...
		sequelize.sync().then(function() {
			...
			var inte_id = jane.get({ plain: true }).id;
			getChild(flowObjs, inte_id);
		});
	...
	function getChild(flowObj, inte_id){
		...
		for( var j=0; j<flowObj.length; j++ ){
			if( flowObj[j].style == "source" || flowObj[j].style == "target" ){
				fillSystem(flowObj[j], inte_id)
			}else if(flowObj[j].style == "microflow") {
				fillMicroflow(flowObj[j], inte_id)
			}
		}
	...
	function fillSystem(flowObj, inte_id){
		sequelize.sync().then(function() {
			return System.create({
				sys_cell_id: flowObj.cell_id,
				integration_id: inte_id,
				sys_name: flowObj.title,
				sys_posX: flowObj.x,
				sys_posY: flowObj.y,
				sys_width: flowObj.width,
				sys_height: flowObj.height,
				sys_style: flowObj.style,
				sys_endpoints: (flowObj.endPoints).join(),
				sys_type: flowObj.type
			});
		}).then(function(jane) {
	...
	function fillMicroflow(flowObj, inte_id){
		sequelize.sync().then(function() {
			return Microflow.create({
				mf_cell_id: flowObj.cell_id,
				integration_id: inte_id,
				mf_name: flowObj.title,
				mf_posX: flowObj.x,
				mf_posY: flowObj.y,
				mf_width: flowObj.width,
				mf_height: flowObj.height,
				mf_style: flowObj.style,
				mf_endpoints: (flowObj.endPoints).join(),
				mf_type: flowObj.type
			});
		}).then(function(jane) {
	...
	app.get('/api/get/projects', function (req, res) {
		...
		var sql = "SELECT * FROM FL_projects";
		con.query(sql, function (err, result) {
			if (err) {
				res.send(err)
			}else{
		...
		con.query(sql, function (err, result) {
			if (err) {
				...
				for(var r=0; r<result.length; r++){
					...
					var sql = "(SELECT mf_cell_id,integration_id,mf_name,mf_posX,mf_posY,mf_width,mf_height,mf_style,mf_type, mf_endpoints FROM FL_microflows WHERE integration_id = '"+cell_id+
					"') UNION (SELECT sys_cell_id,integration_id,sys_name,sys_posX,sys_posY,sys_width,sys_height,sys_style, sys_type, sys_endpoints FROM FL_systems WHERE integration_id = '"+cell_id+"')";
